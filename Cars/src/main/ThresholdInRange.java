...
public class ThresholdInRange {
    ...
    private void update(Mat imgCapture, Mat imgThresh) {
    	BufferedImage imgProc=(BufferedImage)HighGui.toBufferedImage(imgThresh);
    	
    	Point up = new Point(0,imgProc.getHeight()-1);
    	Point down = new Point(0,0);
    	Point left = new Point(imgProc.getWidth()-1,0);
    	Point right = new Point(0,0);
    	
    	for(int j=0; j<imgProc.getHeight();j++)
    		for(int i=0; i<imgProc.getWidth();i++) {
    			if(imgProc.getRGB(i, j)==-1) {
    				if(j<up.getY()) {
    					up=new Point(i,j);
    				}else if(j>down.getY()) {
    					down=new Point(i,j);
    	...
    	Graphics2D g =(Graphics2D)imgProc.getGraphics();
    	g.setColor(Color.RED);
    	g.setStroke(new BasicStroke(5));
    	int x=(int)(left.getX()+(right.getX()-left.getX())/2);
    	int y=(int)(up.getY()+(down.getY()-up.getY())/2);
        ...
        imgDetectionLabel.setIcon(new ImageIcon(imgProc));
        frame.repaint();
    }
